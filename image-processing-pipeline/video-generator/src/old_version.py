...
# NEW: Specific folders for the moderation workflow
GCS_PENDING_FOLDER = "ai-video-quality-tool/output/pending/"
GCS_APPROVED_FOLDER = "ai-video-quality-tool/output/approved/"

# The sync function will now ONLY look in the pending folder for new videos
...
def get_gcs_output_videos(storage_client, bucket_name, prefixes):
    ...
    for prefix in prefixes:
        st.write(f"Scanning: `gs://{bucket_name}/{prefix}`...")
        blobs = storage_client.list_blobs(bucket_name, prefix=prefix)
        for blob in blobs:
            if blob.name.endswith(valid_extensions):
...
def sync_gcs_to_bigquery():
    ...
    # VIDEO_OUTPUT_PREFIXES now points to GCS_PENDING_FOLDER
    ...
    for video_stem, video_path in gcs_output_map.items():
        if video_stem in stem_to_image_id_map:
            ...
            # If BQ row is PENDING, or video_id is blank, update it.
...
def update_decision_in_bq(moderator_id, image_id, decision, new_prompt, new_notes, source_video_path=None):
    ...
    - On 'approve', moves the GCS file from PENDING_FOLDER to APPROVED_FOLDER.
    - On 'regenerate' or 'remove', deletes the GCS file.
    """
    print(f"Updating decision for {image_id} by {moderator_id}: {decision}")
    
    new_video_path = None # Default for regenerate/remove
    new_generation_status = 'MODERATED' # Default for approve/remove
    
    try:
        if decision == 'approve' and source_video_path:
            st.write(f"Approving video. Moving file to '{GCS_APPROVED_FOLDER}'...")
            ...
            st.toast(f"Moved video to: {new_video_path}")

        elif (decision == 'regenerate' or decision == 'remove') and source_video_path:
            st.write(f"Decision '{decision}' requires deletion. Deleting: {source_video_path}")
        ...
        # If decision is 'approve', new_video_path is set.
        # If 'regenerate' or 'remove', it's None.
        
    except Exception as e:
        st.error(f"Error handling GCS file for {source_video_path}: {e}")
        st.stop() # Stop execution if file handling fails

    # Proceed to update BigQuery
    try:
        query = f"""
            UPDATE `{BIGQUERY_TABLE}`
            SET 
        ...
        # Clear cache and update queue
        st.cache_data.clear()
        if 'video_queue' in st.session_state and st.session_state.video_queue:
